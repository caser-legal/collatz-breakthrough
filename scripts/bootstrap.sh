...
export SERVER_NAME=collatz.example.edu
...
# don't forget git clone git@github.com:YOUR_GITHUB_USER/collatz.git into $HOME
SRCDIR=$HOME/collatz/
...
if ! test -d "$SRCDIR"; then
	pushd "$HOME"
	git clone git@github.com:YOUR_GITHUB_USER/collatz.git
	popd
else
	pushd "$SRCDIR"
...
if ! test -d "$MAPDIR"; then
	pushd "$HOME"
	git clone git@github.com:YOUR_GITHUB_USER/collatz-sieve.git
	popd
else
	pushd "$MAPDIR"
	git pull || echo "cannot sync repo"
